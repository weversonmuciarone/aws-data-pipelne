...
class StepInterface:
    ...
    def __init__(self, **config):
        ...
        self.logger = logging.getLogger(f"engine.glue:{self.name}:{self.handler}")  # only used for init in unittests
        self.elapsed = -1
        self.status = StepStatus.PARSED
        self.metrics = []
    ...
    def run(self, spark, config, context, logger, glueContext=None):
        ...
        except PrematureExitException as pe:
    ...
    def premature_exit(self):
        self.status = StepStatus.PREMATURE_EXIT
        self.logger.info(f"{self.name} [{self.type}] EXITED PREMATURELY")
...
class PrematureExitException(Exception):
